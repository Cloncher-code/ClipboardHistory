...
struct GeneralSettingsTab: View {
    ...
    var body: some View {
        Form {
                ...
                Toggle("Запускать при входе в систему", isOn: $launchAtLogin)
                    .onChange(of: launchAtLogin) { _, newValue in
                        do {
                            ...
                            loginErrorText = String(localized: "Не удалось изменить автозапуск: \(error.localizedDescription)")
                        }
                    }
            ...
            Section("Резервная копия") {
                ...
                Text("Полная копия одним zip-файлом: история, списки, заготовки и вложения (картинки, RTF). Пароли в копию не выгружаются.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private func exportBackup() {
        ...
        panel.nameFieldStringValue = "ClipboardHistory-backup.zip"
        panel.allowedContentTypes = [.zip]
        if panel.runModal() == .OK, let url = panel.url {
            backupMessage = manager.exportFullBackup(to: url)
                ? String(localized: "Экспортировано (с вложениями)")
                : String(localized: "Не удалось создать резервную копию")
        }
    }
    ...
    private func importBackup() {
        ...
        if panel.runModal() == .OK, let url = panel.url {
            backupMessage = manager.importFullBackup(from: url)
                ? String(localized: "Импортировано")
                : String(localized: "Не удалось прочитать файл резервной копии")
        }
    }
}
...
struct ExclusionsSettingsTab: View {
    ...
    private func addPattern() {
        manager.addExcludedPattern(newPattern)
        newPattern = ""
    }
    ...
    var body: some View {
        Form {
            ...
            Section("Шаблоны текста") {
                if manager.excludedPatterns.isEmpty {
                    Text("Пока нет шаблонов")
                        .foregroundStyle(.secondary)
                }
                ForEach(manager.excludedPatterns, id: \.self) { pattern in
                    HStack {
                        Text(pattern)
                        ...
                        Spacer()
                        Button {
                            manager.removeExcludedPattern(pattern)
                        } label: {
                            Image(systemName: "trash")
                        }
                        .buttonStyle(.plain)
                ...
                HStack {
                    TextField("Regex или подстрока…", text: $newPattern)
                        .textFieldStyle(.roundedBorder)
                        .onSubmit { addPattern() }
                    Button("Добавить") { addPattern() }
                        .disabled(newPattern.trimmingCharacters(in: .whitespaces).isEmpty)
                }
                Text("Текст, совпавший с шаблоном, не попадает в историю. Работают регулярные выражения (например, ^\\d{16}$ — 16 цифр подряд) или обычные подстроки без учёта регистра.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        VStack(spacing: 12) {
            ...
            Button {
                Task {
                    ...
                    case .newer(let tag):
                        updateStatus = String(localized: "Доступна новая версия: \(tag)")
                        updateAvailable = true
                    case .upToDate:
                        updateStatus = String(localized: "У вас последняя версия")
                    case .noReleases:
                        updateStatus = String(localized: "На GitHub пока нет опубликованных релизов")
                    case .failed(let why):
                        updateStatus = String(localized: "Не удалось проверить: \(why)")
                    }
                    checking = false
                }
            } label: {
